...
Explanation: Compare the next unprocessed arrival time from both logs and append the earlier one to the final log until all entries are
processed*/
#include <stdio.h>

int main() {
    int p, q;
    // Read the size of first log
    scanf("%d", &p);
    int log1[p];
    for (int i = 0; i < p; i++) {
        scanf("%d", &log1[i]);
    }
    // Read the size of second log
    scanf("%d", &q);
    int log2[q];
    for (int i = 0; i < q; i++) {
        scanf("%d", &log2[i]);
